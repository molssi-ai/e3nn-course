...
|---|---|---|
| [01a · Symmetry and equivariance](../notebooks/01a_symmetry_and_equivariance.ipynb) | $\Eg{3}$ and its subgroups $\SOg{3} \subset \Og{3}$; the precise difference between **invariance** (energies) and **equivariance** (forces, dipoles); a numerical demonstration that data augmentation only ever *approximates* a symmetry it never enforces | — |
| [01b · Group representations](../notebooks/01b_group_representations.ipynb) | Why the homomorphism $\Dmat{}(g_1 g_2) = \Dmat{}(g_1)\Dmat{}(g_2)$ is exactly what a layer needs to know about its data; reducible vs. irreducible; the irreps of $\SOg{3}$ labelled by $l$ with dimension $2l+1$; Schur orthogonality; parity | `o3.wigner_D` |
...
| [02b · Spherical harmonics](../notebooks/02b_spherical_harmonics.ipynb) | The equivariant embedding of a *direction*, $\Ylm{l}{}(R\rhat) = \Dmat{l}(R)\,\Ylm{l}{}(\rhat)$; parity $(-1)^l$; the three normalization conventions and when each matters; expanding a function on the sphere, and why truncating at $L$ is a low-pass filter | `o3.spherical_harmonics` |
...
| Lesson | What you learn | Key API |
|---|---|---|
| [05a · Atomistic graphs](../notebooks/05a_atomistic_graphs.ipynb) | Molecules and crystals as radius graphs; the graph-network message-passing framework; the course-wide edge convention $\rvec{ij} = \rvec{j} - \rvec{i}$ and why absolute positions never enter a network; periodic boundary conditions and the extra `edge_shift` periodic edges require | `ase.neighborlist`, `radius_graph` |
...
| Lesson | What you learn | Key API |
|---|---|---|
| [08a · NequIP theory](../notebooks/08a_nequip_theory.ipynb) | The atomic energy ansatz $E = \sum_i E_i$; the interaction block — convolution filters $R(r_{ij})\Ylm{l}{m}(\hat r_{ij})$, tensor-product convolution, self-interaction, ResNet update, gate; how chemistry enters; the paper's central claim that $l>0$ features buy **dramatic data efficiency**, and the experiments behind it | — |
